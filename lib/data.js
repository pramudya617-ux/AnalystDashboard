...
/* DUA lokasi data, bukan satu.
 *
 *   data/ di repo  - data awal yang ikut ter-commit (discord_ringkas.json)
 *   DATA_DIR       - volume permanen Railway, tempat menulis saat berjalan
 *
 * Volume TIDAK dipasang di atas data/ repo: mount akan menutupi isinya dan
 * dashboard menyala tanpa satu pun panggilan. Jadi keduanya hidup berdampingan,
 * persis seperti env_lokal.py di dashboard lama.
 */
const REPO_DATA = path.join(process.cwd(), "data");
const VOLUME = (() => {
  const d = process.env.DATA_DIR;
  try {
    return d && fs.statSync(d).isDirectory() ? d : null;
  } catch {
    return null;
  }
})();
/* Tempat MENULIS: volume kalau ada, kalau tidak folder repo (seperti di laptop). */
export const DATA_TULIS = VOLUME || REPO_DATA;

/* Penarik analis menulis ke volume saat berjalan, jadi salinan di sana lebih
   baru daripada bawaan repo. Repo tetap jadi cadangan untuk deploy pertama,
   saat volume masih kosong. */
const RINGKAS = (() => {
  const vol = path.join(DATA_TULIS, "discord_ringkas.json");
  try {
    return fs.statSync(vol).isFile() ? vol : path.join(REPO_DATA, "discord_ringkas.json");
  } catch {
    return path.join(REPO_DATA, "discord_ringkas.json");
  }
})();
const KOREKSI = path.join(DATA_TULIS, "koreksi_manual.json");
...
/* Koreksi dibaca dari KEDUANYA, bukan salah satu: yang dibuat lewat halaman web
 * tersimpan di volume, sedangkan yang ikut repo datang dari impor atau alat
 * lokal. Kalau hanya satu yang dibaca, salah satu jalur akan tampak tidak
 * berpengaruh - jenis kebingungan yang mahal. Volume menang kalau kuncinya sama,
 * karena itu yang paling baru disunting. */
export function bacaKoreksi() {
  const out = {};
  for (const f of [path.join(REPO_DATA, "koreksi_manual.json"), KOREKSI]) {
    const k = bacaJson(f, null);
    if (k && typeof k === "object") Object.assign(out, k);
  }
  return out;
}

/* MEMBATALKAN koreksi tidak bisa dengan menghapus kuncinya.
 *
 * bacaKoreksi() menggabung DUA berkas, dan yang ditulis hanya volume. Kunci
 * yang asalnya dari salinan repo akan tetap terbaca dari sana walau sudah
 * dibuang dari volume - jadi koreksi yang dibatalkan hidup lagi pada pembacaan
 * berikutnya. Terukur: 11 koreksi hasil impor_koreksi.py ikut repo, dan
 * semuanya berperilaku begitu; tombol Batal tampak bekerja, halaman menyegar,
 * lalu koreksinya masih di situ.
 *
 * Maka pembatalan ditulis sebagai NISAN - { dibatalkan: true } - yang menimpa
 * kunci repo alih-alih menghilang dari sebelahnya. Yang membaca menyaringnya
 * keluar; yang menulis wajib membawanya serta, karena nisan yang ikut terhapus
 * akan membangkitkan lagi apa yang ditandainya.
 */
...
function terapkanKoreksi(c, kor) {
  ...
  /* dibatalkan = NISAN, bukan koreksi. Lihat alasannya di tulisKoreksi(). */
  if (!k || k.dibatalkan) return { ...c, dikoreksi: false };
  return {
    ...c,
    dihapus: !!k.dihapus,
...
export function muatDashboard(mode = "6bulan") {
  ...
  /* zora.json ditulis penjadwal saat berjalan, jadi salinan volume lebih baru
     daripada yang ikut repo. */
  const z = bacaJson(path.join(DATA_TULIS, "zora.json"), null)
         || bacaJson(path.join(REPO_DATA, "zora.json"), null);
  ...
  return {
    ...
    /* Nisan tidak ikut dihitung: yang dibatalkan bukan koreksi yang berlaku. */
    jumlahKoreksi: Object.values(kor).filter((k) => k && !k.dibatalkan).length,
  };
}
