...
/* Barisnya boleh tetap utuh sementara ISINYA hilang. Penarikan 14:11 lolos dari
   penjagaan lama persis begitu: 790 baris masuk, 790 baris keluar, tapi ke-140
   baris futures kehilangan harganya karena Binance sedang membalas 418 untuk
   fapi. Jumlah baris yang punya harga ikut diukur, dan ambangnya ketat: jumlah
   itu praktis tidak pernah turun sendiri, paling-paling sedikit kalau ada koin
   yang delisting. */
const BATAS_SUSUT_BERHARGA = 0.95;
...
function ukur(file) {
  ...
  return {
    baris: baris.length,
    analis: (d.analis || []).length,
    berharga: baris.filter((b) => b && b.masuk !== null && b.masuk !== undefined).length,
  };
}
...
export async function tarikAnalis(alasan = "jadwal") {
  ...
  try {
    ...
    if (sebelum) {
      ...
      const hargaHilang = sesudah.berharga < sebelum.berharga * BATAS_SUSUT_BERHARGA;
      if (analisSusut || barisAnjlok || hargaHilang) {
        fs.copyFileSync(CADANGAN, RINGKAS);
        const pesan =
          `hasil DITOLAK dan dikembalikan: ${sebelum.baris} baris/${sebelum.analis} analis` +
          `/${sebelum.berharga} berharga -> ${sesudah.baris}/${sesudah.analis}/${sesudah.berharga}. ` +
          (hargaHilang
            ? `Harga banyak yang tidak terambil - biasanya Binance membalas 418 untuk futures.`
    ...
    const hasil = `${sesudah.baris} baris, ${sesudah.analis} analis, ${sesudah.berharga} berharga`;
